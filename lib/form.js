...
function donationReceiptHtml(d, names) {
  ...
  const status = official ? 'Official income tax receipt' : 'Donation record'
  const guidance = official
    ? 'Official receipt for income tax purposes. Please retain this copy for your records.'
    : 'This donation has not been issued an official receipt number yet. Print this for your records only — it is not valid for tax purposes until the receipt is officially issued.'
...
export function formDefaults(apiBase, myUid) {
  return { memberNames: {}, memberEmails: {}, userNames: {}, apiBase, myUid }
}
