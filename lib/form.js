...
export function bindForm(s, { api, onSave }) {
  ...
  function syncDonationControls() {
    requestAnimationFrame(() => {
      const donor = document.getElementById('don-donor')
      const method = document.getElementById('don-method')
      const cat = document.getElementById('don-cat')
      const date = document.getElementById('don-date')
  ...
  function resetForm() {
    ...
    s.editId = null; s.expMode = 'create'; s.donMode = 'create'; s.addErr = null; s.detailExp = null; s.detailDon = null
    ...
    s.adding = false; s.delConfirm = false; s.deleting = false; s.actioning = false
  ...
  async function hydrateExpenseSession(id, session) {
    ...
    if (detail) {
      ...
      s.expForm = {
        member_id: detail.member_id || null,
        payee: detail.payee || '',
        amount: rawAmt(detail.amount),
        category: cat,
        expense_date: (detail.expense_date || '').slice(0, 10),
        note: detail.note || detail.description || '',
      }
      s.memberQuery = detail.member_id ? (s.memberNames[detail.member_id] || detail.payee || '') : ''
      s.expHistory = metaEntries(detail, s.userNames, s.myUid)
    }
    s.expApprovals = approvals.items || approvals || []
  }

  function openExpense(x, mode = 'edit') {
    resetForm()
  ...
  s.viewExpense = x => openExpense(x, 'detail')
  s.editExpense = x => openExpense(x, 'edit')
  ...
  s.filterExpensePayees = q => {
    ...
    s.memberHits = (q ? allMembers.filter(m => m.name.toLowerCase().includes(q.toLowerCase())) : allMembers).slice(0, 5)
    const seen = new Set()
  ...
  s.filterVendors = q => s.filterExpensePayees(q)
  s.selectExpMember = m => {
