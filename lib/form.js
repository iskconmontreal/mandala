...
export function bindForm(s, { api, onSave }) {
  ...
  async function hydrateExpenseSession(id, session) {
    ...
    if (detail) {
      ...
      if (!s.expDirty) {
        s.expForm.member_id = detail.member_id || null
        s.expForm.payee = detail.payee || ''
        s.expForm.amount = rawAmt(detail.amount)
        s.expForm.category = cat
        s.expForm.expense_date = (detail.expense_date || '').slice(0, 10)
        s.expForm.note = detail.note || detail.description || ''
        s.memberQuery = detail.member_id ? (detail.payee || s.memberNames[detail.member_id] || '') : ''
        syncExpenseControls()
      }
  ...
  s.viewExpense = x => openExpense(x, 'edit', 'close')
